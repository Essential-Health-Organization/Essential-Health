...
router.post("/register",validInfo, async(req,res)=>{
    try {
        ...
        const user =  await pool.query("SELECT * FROM login_credentials WHERE email = $1",
        [email]);

        //this checks if user exisit

        if(user.rows.length !== 0 ){
            return res.status(401).send("user already exisits")
        }
        // res.json(user.rows)
        ...
        const newUser = await pool.query("INSERT INTO login_credentials(username,email,password)VALUES($1,$2,$3) RETURNING *",[username,email,bcryptPassword])
